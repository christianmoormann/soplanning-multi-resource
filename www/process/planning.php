...
if(isset($_POST['filtreStatutTache'])) {
	...
	$tabCheck = $statuts->getSmartyKeyData('status_id');
	foreach ($_POST['statutsTache'] as $tmpStatut) {
		if(count($filtre) >= $statuts->getCount() || !isset($_POST['statutsTache']) || !array_key_exists($tmpStatut, $tabCheck)) {
			$filtre = array();
		}
	}
	if (!empty($filtre))
	{
		setcookie('filtreStatutTache', base64_encode(json_encode($filtre)), time()+60*60*24*500, '/');	
	}else
	{
		// suppression des cookies
...
if(isset($_POST['filtreStatutProjet'])) {
	...
	$tabCheck = $statuts->getSmartyKeyData('status_id');
	foreach ($_POST['statutsProjet'] as $tmpStatut) {
		if(count($filtre) >= $statuts->getCount() || !isset($_POST['statutsProjet']) || !array_key_exists($tmpStatut, $tabCheck)) {
			$filtre = array();
		}
	}
	...
	if (!empty($filtre))
	{
		setcookie('filtreStatutProjet', base64_encode(json_encode($filtre)), time()+60*60*24*500, '/');	
	}else
	{
		// suppression des cookies
