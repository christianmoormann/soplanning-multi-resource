...
function restore_taches()
{
	...
	while ( ($data = fgetcsv($handle, 1000, ";") ) !== FALSE ) {
		...
		$periode->titre = ($data[9] != '' ? xss_save_db(substr($data[9],0, $periode->getFieldSize('titre'))) : null);
		$periode->notes = ($data[10] != '' ? xss_save_db(substr($data[10],0, $periode->getFieldSize('notes'))) : null);
...
function restore_lieux()
{
	...
	while ( ($data = fgetcsv($handle, 1000, ";") ) !== FALSE ) {
		...
		// on ajoute ou met à jour les données
		$lieu->lieu_id = $data[0];
		$lieu->nom = ($data[1] != '' ? xss_save_db(substr($data[1],0, $lieu->getFieldSize('nom'))) : null);
		$lieu->commentaire = ($data[2] != '' ? xss_save_db(substr($data[2],0, $lieu->getFieldSize('commentaire'))) : null);
...
function restore_ressources()
{
	...
	while ( ($data = fgetcsv($handle, 1000, ";") ) !== FALSE ) {
		...
		$ressource->nom = ($data[1] != '' ?  xss_save_db(substr($data[1],0, $ressource->getFieldSize('nom'))) : null);
		$ressource->commentaire = ($data[2] != '' ? xss_save_db(substr($data[2],0, $ressource->getFieldSize('commentaire'))) : null);
