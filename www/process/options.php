...
if(isset($_POST['SOPLANNING_OPTION_AUDIT_RETENTION'])) {
	...
	$retentionDays = filter_var($_POST['SOPLANNING_OPTION_AUDIT_RETENTION'], FILTER_VALIDATE_INT, ['options' => ['min_range' => 1, 'max_range' => 36500]]);
	if ($retentionDays === false) {
		$_SESSION['erreur'] = 'changeNotOK';
		header('Location: ../options' . (isset($_POST['tab']) ? '?tab=' . $_POST['tab'] : ''));
		exit;
	}
	$config->valeur = (string)$retentionDays;
	if(!$config->db_save()) {
		$_SESSION['erreur'] = 'changeNotOK';
		header('Location: ../options' . (isset($_POST['tab']) ? '?tab=' . $_POST['tab'] : ''));
		exit;
